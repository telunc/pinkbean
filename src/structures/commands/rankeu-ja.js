...
export default async(tokens, message) => {
    ...
    let character = await rp({ uri: `${config.get('server')}/api/rank/eu/${name}/overall`, json: true }).catch(() => {
    ...
    let characters = await rp({ uri: `${config.get('server')}/api/rank/eu/${name}/job-alliance`, json: true }).catch(() => {
        // console.error(`failed to load character with name ${name}`);
    });

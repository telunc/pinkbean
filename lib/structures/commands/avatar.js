...
async function avatarChat(message) {
    ...
    return message.channel.send('<' + _config2.default.get('server') + '/api/avatar/' + avatar.id + '/chat/' + message.author.username + '>');
}
...
async function avatarHelp(message) {
    return message.channel.send('', {
        embed: {
            ...
            fields: [{ name: '!avatar', value: 'Display your avatar / show this command' }, { name: '!avatar init (color)', value: 'Choose one of the following colors to initiate your avatar base\n`light`, `ashen`, `pale pink`, `tanned`, `pale`, `green`, `ghostly`, `dark`, `clay`, `mercedes`' }, { name: '!avatar add (item)', value: 'Search and equip your avatar' }, { name: '!avatar delete', value: 'Delete your avatar' }, { name: '!avatar download', value: 'Download your avatar' }, { name: '!avatar chat', value: 'Play with Henesys.Chat' }, { name: '!avatar [@mention]', value: 'Display your mentioned buddy\'s avatar' }, { name: '!avatar random', value: 'Display a random avatar' }, { name: '!avatar help', value: 'Display this message' }, { name: '!avatar design', value: 'Pink Bean: Design' }]
        }
    });
}

...
async function avatar(message) {
    ...
    return await message.channel.send({
        files: [{ attachment: _config2.default.get('server') + '/api/avatar/' + avatar.id + '/icon', name: 'avatar.png' }]
    }).catch(function () {
...
async function avatarAdd(tokens, message) {
    ...
    return await message.channel.send({
        files: [{ attachment: _config2.default.get('server') + '/api/avatar/' + message.author.id + '/icon', name: 'avatar.png' }]
    }).catch(function () {
        message.channel.send('Failed to load avatar resource');
    });
}
...
async function avatarHelp(message) {
    return message.channel.send('', {
        embed: {
            ...
            fields: [{ name: '!avatar', value: 'Display your avatar / show this command' }, { name: '!avatar init (color)', value: 'Choose one of the following colors to initiate your avatar base\n`light`, `ashen`, `pale pink`, `tanned`, `pale`, `green`, `ghostly`, `dark`, `clay`' }, { name: '!avatar add (item)', value: 'Search and equip your avatar' }, { name: '!avatar delete', value: 'Delete your avatar' }, { name: '!avatar download', value: 'Download your avatar' }, { name: '!avatar [@mention]', value: 'Display your mentioned buddy\'s avatar' }, { name: '!avatar random', value: 'Display a random avatar' }, { name: '!avatar help', value: 'Display this message' }]
        }
    });
}
...
async function avatarMention(message) {
    ...
    return await message.channel.send({
        files: [{ attachment: _config2.default.get('server') + '/api/avatar/' + member.id + '/icon', name: 'avatar.png' }]
    }).catch(function () {
        message.channel.send('Failed to load avatar resource');
    });
}
